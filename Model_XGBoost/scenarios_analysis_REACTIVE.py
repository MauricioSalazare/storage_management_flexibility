...
#
#     pickle.dump((scenario_list, model_results), open(abs_path / 'XGBoost_scenario_analysis_REACTIVE.dat', 'wb'))
# else:
#     (scenario_list, model_results) = pickle.load(open(abs_path / 'XGBoost_scenario_analysis_REACTIVE.dat', 'rb'))
#
# #%%
...
# opt_params = model_results['best_model'][idx_[1]].get_params()

# Load the best hyperparameters from the nested cross validation on 200 scenarios.
...
if RETRAIN_ALL_MODELS:
    ...
    pickle.dump((scenario_list_final, cross_val_results), open(abs_path / 'XGBoost_scenario_model_REACTIVE.dat', 'wb'))
else:
    (scenario_list_final, cross_val_results) = pickle.load(open(abs_path / 'XGBoost_scenario_model_REACTIVE.dat', 'rb'))
...
plt.subplots_adjust(bottom=0.22, top=0.98, right=0.98, left=0.1)
...
ax.set_ylabel('NRMSE [\%]', fontsize=7)
ax.set_xlabel('Number of scenarios', fontsize=7)
