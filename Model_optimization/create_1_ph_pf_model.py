...
def create_1_ph_pf_model(Vnom, Snom, Vmin, Vmax, Data_Network):
    ...
    def QDs_init_rule(model,i,t,s):
        return model.PDs[i, t, s]*tan(atan(model.QD[i]/(model.PD[i]+1e-32)))
    ...
    def act_loss(model):
        return (1/len(Os)*sum(sum(sum(model.RM[i,j]*(model.I[i,j,t,s]) for i,j in model.Ol) for t in model.OT) for s in model.Os))
    model.obj = Objective(rule=act_loss)


    
    def active_power_flow_rule(model, k,t,s):
            return (sum(model.P[j,i,t,s] for j,i in model.Ol if i == k ) - sum(model.P[i,j,t,s] + model.RM[i,j]*(model.I[i,j,t,s]) for i,j in model.Ol if k == i) + model.PS[k,t,s] == model.PDs[k,t,s])
    ...
    def reactive_power_flow_rule(model, k,t,s):
            return (sum(model.Q[j,i,t,s] for j,i in model.Ol if i == k ) - sum(model.Q[i,j,t,s] + model.XM[i,j]*(model.I[i,j,t,s]) for i,j in model.Ol if k == i) + model.QS[k,t,s] == model.QDs[k,t,s])
    ...
    def voltage_drop_rule(model, i, j,t,s):
        return (model.V[i,t,s] - 2*(model.RM[i,j]*model.P[i,j,t,s] + model.XM[i,j]*model.Q[i,j,t,s]) - (model.RM[i,j]**2 + model.XM[i,j]**2)*model.I[i,j,t,s] - model.V[j,t,s] == 0)
